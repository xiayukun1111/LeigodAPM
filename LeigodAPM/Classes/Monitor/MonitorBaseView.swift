...
extension MonitorBaseView {
    fileprivate func attributes(size:CGFloat) -> [NSAttributedString.Key : Any] {
        return [NSAttributedString.Key.font:UIFont.helvetica(size),
...
extension MonitorBaseView {
    func configure(fps: Double) {
        EyesManager.shared.fps = String(format: "FPS\n %.1f", fps)
        self.contentLabel.attributedText = self.contentString(String(format: "%.1f", fps), unit: "FPS")
    }
}
